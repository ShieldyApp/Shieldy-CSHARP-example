...
namespace ShieldyCSharpExample
{
    public class ShieldyApi
    {
        ...
        public string Xor(string val, string key)
        {
        ...
        public string Xor(string val, IList<byte> key)
        {
            var result = new StringBuilder();
            for (var i = 0; i < val.Length; i++)
            {
        ...
        public List<byte> Xor(List<byte> toEncrypt, string xorKey)
        {
        ...
        private bool CompareBytearrays(ICollection<byte> a, IList<byte> b)
        {
            if (a.Count != b.Count)
                return false;
            var i = 0;
        ...
        public bool Initialize(string appGuid, string version, string appSalt)
        {
            ...
            Initialized = Bindings.SC_Initialize(appGuid, version);
            return Initialized;
        }
        ...
        public string GetVariable(string name)
        {
            ...
            if (result)
            {
                ...
                return Xor(resultVariable, AppSalt);
            }
        ...
        public List<byte> DownloadFile(string name)
        {
            ...
            if (result)
            {
                ...
                return Xor(data.ToList(), AppSalt);
            }
